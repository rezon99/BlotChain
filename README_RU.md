...
### 🤖 Telegram Bot & Служба автоматизации
Интерактивный Telegram-бот (`automation/bot/src/telegramBot.ts`) работает в режиме polling и поддерживает команды `/mint`, `/burn`, `/status` и `/balance`.

> **Обратите внимание:** Сервис Telegram-бота с постоянным опросом (polling) требует хостинга с непрерывным исполнением процессов (например, VPS, Railway, Render, Fly.io). Среды GitHub Actions не подходят для запуска долгоживущего слушателя polling, так как они завершаются сразу после выполнения задачи.
